...
#define DEFAULT_WIDTH   1280        // AL defines the required width
#define DEFAULT_HEIGHT  960         // AL defines the required height
...
static int mxc_streamoff(cam_data *cam)
{
    ...
    printk("%s: ipu%d/csi%d capture_on=%d %s\n", __func__, cam->ipu_id,
...
static int mxc_v4l_dqueue(cam_data *cam, struct v4l2_buffer *buf)
{
	...
	if (!wait_event_interruptible_timeout(cam->enc_queue,
		...
		pr_err("ERROR: v4l2 capture: mxc_v4l_dqueue timeout "
			"enc_counter %x\n",
		       cam->enc_counter);
		printk("ERROR: v4l2 capture: mxc_v4l_dqueue timeout "
			"enc_counter %x\n",
		       cam->enc_counter);
		return -ETIME;
		...
		pr_err("ERROR: v4l2 capture: mxc_v4l_dqueue() "
			"interrupt received\n");
		printk("ERROR: v4l2 capture: mxc_v4l_dqueue() "
			"interrupt received\n");
		return -ERESTARTSYS;
	}
	...
	if (frame->buffer.flags & V4L2_BUF_FLAG_DONE) {
		...
		pr_err("ERROR: v4l2 capture: VIDIOC_DQBUF: "
			"Buffer not filled.\n");
		printk("ERROR: v4l2 capture: VIDIOC_DQBUF: "
			"Buffer not filled.\n");
...
void power_up_camera(cam_data *cam)
{
	...
	printk("PEAR: power_up_camera runs\n");
...
static int mxc_v4l_open(struct file *file)
{
	...
	printk("ORANGE: mxc_v4l2_open starts\n");
	...
	if (cam->open_count++ == 0) {
		...
		printk("ORANGE: mxc_v4l2_open gpr pointer value = %x\n", err);
		...
		if (!IS_ERR(gpr)) {
			if (of_machine_is_compatible("fsl,imx6q")) {
				if (cam->ipu_id == cam->csi) {
					...
					printk("ORANGE: mxc_v4l2_open imx6q mask = %x, val = %x\n", mask, val);
				}
				...
				printk("ORANGE: mxc_v4l2_open imx6dl mask = %x, val = %x, shift = %x, cam->csi = %x, csi_bit = %x\n", mask, val, shift, cam->csi, csi_bit);
			...
			printk("ORANGE: mxc_v4l2_open failed to find compatible architecture\n");
...
oops:
	...
	printk("ORANGE: mxc_v4l2_open returned with %d", err);
	return err;
}
...
static int mxc_v4l_close(struct file *file)
{
	...
	if (--cam->open_count == 0) {
        ...
        pr_err("close input type %s", mxc_capture_inputs[cam->current_input].name);
			    ...
			    unsigned int ioctlnr, void *arg)
{
	...
	int retval = 0;
	unsigned long lock_flags;
    static struct timeval Interval = { 0 };
    struct timeval Now;
	...
	case VIDIOC_DQBUF: {
        ...
        /*
        do_gettimeofday(&Now);
        printk("%s Interval %d.%d\n", __func__, Now.tv_sec - Interval.tv_sec, Now.tv_usec - Interval.tv_usec);
...
static unsigned int mxc_poll(struct file *file, struct poll_table_struct *wait)
{
    ...
    printk("%s queue %x, wait %x, wait._qproc %x", __func__, queue, wait, wait->_qproc);
...
static int init_camera_struct(cam_data *cam, struct platform_device *pdev)
{
    ...
    //if (ret || (def_input != 0 && def_input != 1)) // AL removed to force the Image Converter not to be used
        def_input = 1;
	...
	*(cam->video_dev) = mxc_v4l_template;
    ...
    cam->crop_bounds.width = DEFAULT_WIDTH;
	cam->crop_bounds.top = 0;
    cam->crop_bounds.height = DEFAULT_HEIGHT;
    ...
    cam->standard.framelines = DEFAULT_HEIGHT;
