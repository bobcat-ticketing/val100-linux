...
/*ASa: Add an hexDump function for debug*/
...
dhdsdio_download_nvram(struct dhd_bus *bus)
{
	...
	/* Download variables */
	if (nvram_file_exists) {
DHD_ERROR(("%s: nvram file exist\n", __FUNCTION__));
		len = dhd_os_get_image_block(memblock, MAX_NVRAMBUF_SIZE, image);
	}
	else {
DHD_ERROR(("%s: nvram does NOT file exist\n", __FUNCTION__));
...
//	DHD_INFO(("%s: bus:%p, mem:%p len:%x ret=%d\n", __FUNCTION__
	DHD_INFO(("%s: bus:%p, mem:%p len:%d ret=%d\n", __FUNCTION__
